...
-	OpenAPI — это сам формат спецификации (стандарт OAS 3.x);
-	Swagger — это набор инструментов для работы с этой спецификацией (Swagger UI, Swagger Editor, Swagger Codegen, swagger-cli и др.).
3.	В чём различие подходов code-first и schema-first? Плюсы/минусы.
4.	Какие обязательные разделы содержит спецификация OpenAPI?
5.	Для чего нужны components.schemas и как их переиспользовать в responses?
6.	Что описывают аннотации @Param, @Success, @Failure, @Router, @Security?
7.	Как опубликовать Swagger UI на отдельном префиксе (/docs) и ограничить к нему доступ?
8.	Как поддерживать актуальность документации при изменениях кода?
9.	Как подключить Bearer-аутентификацию в спецификации и что изменится в UI?
